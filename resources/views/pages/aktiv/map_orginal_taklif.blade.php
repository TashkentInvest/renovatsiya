...
<head>
    ...
    *, *::before, *::after {
    ...
    @media (max-width: 992px) {
        ...
        .map-controls, .map-style-controls {
            position: relative;
            top: auto;
            left: auto;
            right: auto;
...
<body>
    ...
    <div class="top-notice-bar">
        <div class="container">
            ...
            <span>Ўзбекистон Республикаси расмий инвестиция харитаси - Барча маълумотлар ҳақиқий вақтда янгиланади</span>
        </div>
    </div>
...
<div class="popup-info"><strong>Бошланғич нархи:</strong> ${lot.start_price ? Number(lot.start_price).toLocaleString('uz-UZ') : 'Белгиланмаган'} сўм</div>
                ...
                <div class="popup-buttons">
                    ${lot.lot_link && lot.lot_link !== '#' ? `<a href="${lot.lot_link}" target="_blank" class="popup-btn external">
                        <i class="fas fa-external-link-alt"></i> Батафсил
                    </a>` : ''}
                </div>
            </div>
        `;
        ...
        AppState.auctionMarkers.push({ marker, data: lot });
    ...
    addApiMarker(lot) {
        ...
        AppState.markers.push({ marker, data: lot });
        ...
        const kmzLayer = L.geoJSON(geoJson, {
            ...
            onEachFeature: function(feature, layer) {
                ...
                if (feature.properties && feature.properties.description) {
                    const parsedData = Utils.parseDescriptionData(feature.properties.description);
                ...
                if (layer.setStyle) {
                    layer.on('mouseover', function() {
                        this.setStyle({ weight: 5, fillOpacity: 0.6 });
                    });
...
const UI = {
    ...
    updateControls() {
        ...
        const total = AppState.counts.regular + AppState.counts.auction + AppState.counts.jsonData + AppState.counts.kmz + AppState.counts.dopKmz;
    ...
    updateButtonStates() {
        ...
        if (jsonBtn) {
            ...
            if (span) {
                span.textContent = AppState.visibility.jsonData ? 'JSON маълумотларни яшириш' : 'JSON маълумотларни кўрсатиш';
            }
        ...
        if (auctionBtn) {
            ...
            if (span) {
                span.textContent = AppState.visibility.auction ? 'Аукционларни яшириш' : 'Аукционларни кўрсатиш';
            }
            auctionBtn.className = AppState.visibility.auction ? 'map-control-btn auction-active' : 'map-control-btn';
        }
    ...
    showDopKmzModal(fileName) {
        ...
        Object.keys(lotData).forEach(key => {
            if (key !== 'Номи') {
                modalContent += `<tr><td><i class="fas fa-info"></i> ${key}:</td><td>${lotData[key]}</td></tr>`;
            }
        });
...
const DataLoader = {
    ...
    async loadAllData() {
        ...
        try {
            // Reset counts
            AppState.counts = { regular: 0, auction: 0, jsonData: 0, kmz: 0, dopKmz: 0 };
...
const Utils = {
    ...
    getStatusInfo(item) {
        ...
        switch (type) {
            case 'Реновация':
                return { text: 'Реновация', class: 'badge-renovation', color: '#8b5cf6' };
            case 'Инвестиция':
                return { text: 'Инвестиция', class: 'badge-investment', color: '#0ea5e9' };
            case 'Аукцион':
                return { text: 'Аукцион', class: 'badge-auction', color: '#ea580c' };
            default:
                return { text: type || 'Белгисиз', class: 'badge-info', color: '#3b82f6' };
        }
    },
     ...
     */
    formatStatus(status) {
        if (!status) return { text: "Статус не указан", class: "badge-info" };

        switch (status) {
            case "9":
                return { text: "Инвест договор", class: "badge-success" };
            case "1":
                return { text: "Ишлаб чиқилмоқда", class: "badge-warning" };
            case "2":
                return { text: "Қурилиш жараёнида", class: "badge-info" };
            default:
                return { text: "Статус: " + status, class: "badge-info" };
        }
    },
...
const DataServices = {
    ...
    async fetchApiData() {
        try {
            ...
            return { success: true, data: lotsData };
        } catch (error) {
    ...
    async processDopKmzFile(fileName) {
        try {
            ...
            return { success: true, geoJson, fileName };
        } catch (error) {
...
const MapManager = {
    ...
    initMap() {
        ...
        AppState.mapLayers.satellite = L.tileLayer('https://server.arcgisonline.com/ArcGIS/rest/services/World_Imagery/MapServer/tile/{z}/{y}/{x}', {
            attribution: '© Esri'
        });

        AppState.mapLayers.hybridBase = L.tileLayer('https://server.arcgisonline.com/ArcGIS/rest/services/World_Imagery/MapServer/tile/{z}/{y}/{x}', {
            attribution: '© Esri'
        });

        AppState.mapLayers.hybridLabels = L.tileLayer('https://server.arcgisonline.com/ArcGIS/rest/services/Reference/World_Boundaries_and_Places/MapServer/tile/{z}/{y}/{x}', {
            attribution: ''
        });
    ...
    addJsonDataMarker(item) {
        ...
        AppState.jsonDataMarkers.push({ marker, data: item });
